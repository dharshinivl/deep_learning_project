...
app = FastAPI(title="Deepfake Detector", version="0.1")
...
def index_page():
    return HTMLResponse(
        """
        <html><head><title>Deepfake Detector</title></head>
        <body style='font-family:sans-serif;max-width:720px;margin:40px auto;'>
          <h2>Deepfake Detector</h2>
          <p>Upload an image or a short video. Optional heatmap overlay for images.</p>
          <form action="/predict-image" method="post" enctype="multipart/form-data">
            <div><input type="file" name="file" accept="image/*" required></div>
            <label><input type="checkbox" name="heatmap" value="true"> return heatmap</label>
            <div><button type="submit">Predict Image</button></div>
          </form>
          <hr/>
          <form action="/predict-video" method="post" enctype="multipart/form-data">
            <div><input type="file" name="file" accept="video/*" required></div>
            <div><button type="submit">Predict Video</button></div>
          </form>
        </body></html>
        """
    )
...
async def predict_image_endpoint(request: Request, file: UploadFile = File(...), heatmap: Optional[bool] = False):
    ...
    resp = {"prob": prob, "label": result["label"], "threshold": THRESHOLD, "no_face_detected": result.get("no_face_detected", False)}
...
# Minimal video endpoint: process first middle frame
...
async def predict_video_endpoint(request: Request, file: UploadFile = File(...), heatmap: Optional[bool] = False):
    ...
    resp = {"prob": prob, "label": label, "threshold": THRESHOLD, "frames": len(probs)}
